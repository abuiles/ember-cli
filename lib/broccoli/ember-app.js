...
var broccoliLeaf = require('broccoli-leaf');
...
EmberApp.prototype.import = function(asset, options) {
  ...
  if (options.amd) {
    if (!extension) {
      // if dir and amd then  process all files and append them as
      // legacy files for now let's suppose it is anom
      //

      // deanonimize all files in assetPath dir see https://github.com/abuiles/broccoli-leaf/blob/master/Brocfile.js#L3
      var assetTree = broccoliLeaf(assetPath, {
        name: options.remap
      });
      ...
      // Append every e.g.
      //  bower_components/ember-qunit/dist/amd/config.js',
      //  bower_components/ember-qunit/dist/amd/initializers',
      //  bower_components/ember-qunit/dist/amd/main.js',
      //  bower_components/ember-qunit/dist/amd/service.js'
      //

      fs.readdirSync(assetPath).forEach(function(file) {
        legacyFilesToAppend.push([assetPath, '/', file].join(''));
      });

      // detect exports with leaf and add them to importWhitelist
    } else {
      // Handle files e.g. moment.js
